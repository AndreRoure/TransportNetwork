...
global{
	map road_filtering <- map(["highway"::["primary", "secondary", "tertiary", "motorway", "living_street", "residential", "unclassified", "road", "trunk", "motorway_link", "trunk_link", "primary_link", "secondary_link", "tertiary_link"]]);
	
	
	file<geometry> road_osm <-  file<geometry>(osm_file("../includes/brasilia.osm", road_filtering))  ;
	
	geometry shape <- envelope(road_osm);
	graph the_graph; 
	...
	init {
		int count <- 0;
		write "OSM file loaded: " + length(road_osm) + " geometries";
		
//		from the OSM file, creation of the selected agents
		loop geom over: road_osm {
			if (shape covers geom) {
				...
				if (length(geom.points) = 1 ) {
					...
					string osm_id <- string(geom get ("osm_id"));
					if (osm_id = nil){
						count <- count+1;
					}
					int lanes_val <- empty(lanes_str) ? 1 : ((length(lanes_str) > 1) ? int(first(lanes_str)) : int(lanes_str));
					create road with: [shape ::geom, id::osm_id, type:: highway_str, oneway::oneway, maxspeed::maxspeed_val, lanes::lanes_val] {
		...
		save road type:"shp" to:"../includes/roads.shp" attributes:["lanes"::self.lanes, "maxspeed"::maxspeed, "oneway"::oneway, "id"::id] ;
